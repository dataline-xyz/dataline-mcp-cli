...
import { DataApiError } from "../../../src/data-api/error.js";
...
describe("Dataline x402 fetch", () => {
  ...
  it("preserves the payment rejection returned after a signed retry", async () => {
    const initialChallenge = paymentRequired();
    const rejectedChallenge = paymentRequired({
      error: "Facilitator verify failed: insufficient Base USDC balance.",
    });
    const upstreamFetch = vi.fn<typeof globalThis.fetch>((input, init) => {
      const headers = new Headers(init?.headers ?? (input instanceof Request ? input.headers : {}));
      const challenge = headers.has("payment-signature") ? rejectedChallenge : initialChallenge;
      return Promise.resolve(
    ...
    const error = await client
      .get("/crypto/cex/price", { base: "BTC" })
      .catch((caught: unknown) => caught);
    expect(error).toBeInstanceOf(DataApiError);
    if (!(error instanceof DataApiError)) {
      throw new Error("Expected the x402 rejection to become a DataApiError.");
    }
    expect(error).toMatchObject({
      code: "x402_payment_rejected",
      message: "x402 payment rejected: Facilitator verify failed: insufficient Base USDC balance.",
      retryable: false,
      status: 402,
    });
  ...
  it("preserves a local payment-policy rejection before signing", async () => {
    const challenge = paymentRequired({ accepts: [requirement({ amount: "1001" })] });
    ...
    expect(error).toBeInstanceOf(DataApiError);
    if (!(error instanceof DataApiError)) {
      throw new Error("Expected the x402 policy rejection to become a DataApiError.");
    }
    expect(error).toMatchObject({
      code: "x402_payment_not_allowed",
      message:
        "The payment challenge did not offer exact USDC on eip155:8453 within the 0.001 USD limit.",
      retryable: false,
    });
    expect(typeof error.requestId).toBe("string");
